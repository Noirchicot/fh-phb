# Feats

Fate's Hand tweaks a handful of feats. Each entry shows its FH5+ rules; unfold *view older version* for the original. Anything not shown here works exactly as printed.

## Origin feats

Your **Inheritance** grants one origin feat, **your choice**, from these five:

| Origin feat | What it does in Fate's Hand |
|---|---|
| **Alert** | as printed |
| **Magic Initiate** | as printed |
| **Savage Attacker** | as printed |
| **Skilled** | ⭐ **+6 free skill points** — the SRD grants 3 proficiencies, and a proficiency is worth 2 points in Fate's Hand (see [Skills & Tools — Player Guide](skills-and-tools.md)) |
| **Auspicious** *(FH)* | **+2 to your Destiny Score** — see below |

> ⛔ **No background record grants a feat.** There are no backgrounds in Fate's Hand; the step is
> called **Inheritance** and the feat is always chosen freely. See [Inheritance](inheritance.md).

## Skill feats

### [Observant](https://www.dndbeyond.com/sources/dnd/phb-2024/feats#Observant)
<span class="fh-edition">FH5+ — General feat (level 4+, INT or WIS 13+).</span>

- Increase your Intelligence or Wisdom by 1, to a maximum of 20.
- Take the Search action as a Bonus Action.
- Add your Intelligence or Wisdom modifier to Survival, Delve and Vigilance checks.

<details class="fh-removed">
<summary>view older version</summary>
<div>Ability Score Increase: +1 Intelligence or Wisdom (max 20).</div>
<div>Quick Search: take the Search action as a Bonus Action.</div>
</details>

### [Keen Mind](https://www.dndbeyond.com/sources/dnd/phb-2024/feats#KeenMind)
<span class="fh-edition">FH5+ — General feat (level 4+, INT 13+).</span>

- Increase your Intelligence by 1, to a maximum of 20.
- Take the Study action as a Bonus Action.
- Add your Intelligence or Wisdom modifier to Insight and Investigation checks.

<details class="fh-removed">
<summary>view older version</summary>
<div>Ability Score Increase: +1 Intelligence (max 20).</div>
<div>Quick Study: take the Study action as a Bonus Action.</div>
</details>

### Auspicious *(FH)*
<span class="fh-edition">FH5+ — Origin feat.</span>

- +2 to your Destiny Score.

<details class="fh-removed">
<summary>view older version</summary>
<div>Luck Points equal to your proficiency bonus (regained on a long rest).</div>
<div>Spend 1: Advantage on a d20 roll, or impose Disadvantage on an attack roll against you.</div>
</details>

## Combat feats

### [Spell Sniper](https://www.dndbeyond.com/feats/1789201-spell-sniper)
<span class="fh-edition">FH5+ — General feat (level 4+, a spellcasting feature).</span>

- Increase your Intelligence, Wisdom or Charisma by 1, to a maximum of 20.
- No Disadvantage for casting within 5 ft of an enemy.
- A spell that requires an attack roll gains +60 ft range.
- Critical range +1.

<details class="fh-removed">
<summary>view older version</summary>
<div>Ability Score Increase: +1 Intelligence, Wisdom or Charisma (max 20).</div>
<div>Bypass Cover: your spell attack rolls ignore Half and Three-Quarters Cover.</div>
<div>Casting in Melee: no Disadvantage on spell attack rolls while within 5 ft of an enemy.</div>
<div>Increased Range: a spell that needs an attack roll (range 10 ft+) gains +60 ft range.</div>
</details>

### [Great Weapon Fighting](https://www.dndbeyond.com/feats/1789148-great-weapon-fighting)
<span class="fh-edition">FH5+ — Fighting Style feat.</span>

- Add your proficiency bonus to the damage (it goes into "graze").

<details class="fh-removed">
<summary>view older version</summary>
<div>When you roll damage for a Melee weapon wielded in two hands, treat a 1 or 2 on a damage die as a 3.</div>
</details>

### [Great Weapon Master](https://www.dndbeyond.com/feats/1789149-great-weapon-master)
<span class="fh-edition">FH5+ — General feat (level 4+, STR 13+).</span>

- Increase your Strength score by 1, to a maximum of 20.
- **Heavy Weapon Mastery**: when attacking with a Heavy weapon, provided you did not move this round, add your proficiency bonus to the damage (this stacks with Great Weapon Fighting).
- **Hew**: after a Critical Hit or a kill with a melee weapon, make one Bonus-Action attack.

<details class="fh-removed">
<summary>view older version</summary>
<div>Ability Score Increase: +1 Strength (max 20).</div>
<div>Heavy Weapon Mastery: when you hit with a Heavy weapon as part of the Attack action, add your proficiency bonus to the damage (no condition, no Great Weapon Fighting stacking).</div>
<div>Hew: after a Critical Hit or a kill with a melee weapon, make one Bonus-Action attack.</div>
</details>

### [Sharpshooter](https://www.dndbeyond.com/feats/1789187-sharpshooter)
<span class="fh-edition">FH5+ — General feat (level 4+, DEX 13+).</span>

- Increase your Dexterity by 1, to a maximum of 20.
- Ranged weapon attacks ignore Half and Three-Quarters Cover.
- No Disadvantage within 5 ft of an enemy.
- No Disadvantage at long range.
- Add your proficiency bonus to the damage.
- Firing into a melee can't accidentally hit your allies.

<details class="fh-removed">
<summary>view older version</summary>
<div>Ability Score Increase: +1 Dexterity (max 20).</div>
<div>Bypass Cover: your ranged weapon attacks ignore Half and Three-Quarters Cover.</div>
<div>Firing in Melee: no Disadvantage on ranged attack rolls while within 5 ft of an enemy.</div>
<div>Long Shots: no Disadvantage on ranged attack rolls at long range.</div>
</details>

## Command feats

### Battlefield Tactician
<span class="fh-edition">FH5+ — General feat (level 4+, CHA 13+).</span>

- +1 to INT, WIS or CHA.
- Help, Study, tactical, and Leadership actions take a bonus action instead of an action.
- **Issue Orders:** a CHA [Leadership](skills-and-tools.md#leadership) order — DC 12, affecting all willing targets within 30 ft.
    - You affect +1 person per 2 points over the DC (12 = 1, 14 = 2, 16 = 3…).
    - Affecting more than 1 person costs 1 Destiny Point. Both leader and target must be at least **Novice** in the directed skill, attack or spell.

### Battlefield Master
<span class="fh-edition">FH5+ — General feat (level 4+, CHA 13+). Prerequisite: Battlefield Tactician — improves it.</span>

- Enables mass Leadership: you lead other *leaders* rather than rank-and-file allies.
- Valid sub-leaders are only those who are themselves Battlefield Tacticians or have Expertise in the directed course of action, and you must be able to see each other.
- The order is a DC 12 Leadership check, affecting one additional group per increment of 2 (12 = 1 group, 14 = 2, 16 = 3…).
- On a success, each sub-leader gains a synergy bonus on the Leadership rolls they apply to their own group, for one minute.
- Full Leadership mechanics: see the [Leadership rules](skills-and-tools.md#leadership).

## Soulforge feats

### Body Forge Adept
<span class="fh-edition">FH5+ — General feat.</span>

- +1 Constitution or Wisdom *(will)* (max 20).
- You gain a **4th attunement slot** that can hold **only a body-forged gem** (see [Body Forging](soulforge-crafting.md#body-forging-fuse-a-gem-into-your-flesh)) — letting you bear a **second** gem in your flesh.
- *No feat is needed for your first body-forged gem — anyone may bear one, in a normal attunement slot.*

**Attunement at a glance**

- *Without the feat:* slots 1–3 — each may hold a normal magic item, a Soulforged item, or your body-forged gem.
- *With the feat:* slots 1–3 as above · slot 4 — **body-forged gem only**.