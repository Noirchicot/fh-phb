...
There is no negative edge. A penalty is never an edge taken away — it is an edge given to the other side: an edge on the defender's AC, or an edge on the DC, which is +2 to the DC. Nothing subtracts. The reason is at the table rather than in the arithmetic: nothing accumulates, so nobody has to add anything up.

**A passive, not an edge.** An edge is something someone does; a passive is a property that is there. A strangled defender does not grant their +2 AC — they have it, because they are being strangled. The distinction decides whether a +2 counts toward Advantage or simply sits on the AC like a piece of armour.

Two more things keep their numbers, deliberately. **Exhaustion** stays a flat −1 per degree: it is not situational, and it is already right. The −2 of the [Chaos Tables](chaos-tables.md) are lasting harm to an ability score, not modifiers on a roll.

## Why the DCs are higher

At levels 1–4 the base **DC** for a challenging check is 16, rising with tier after that (the GM's tiered table). This is not a difficulty setting — it is the counterweight to everything above. Fate's Hand gives you more ways to lean a roll than the base game does, so it asks a little more of the roll: a single edge turns a DC 16 into a 14 in practice. Raise the ceiling, raise the floor.
