...
(function () {
  ...
  /* ── Modules sortis (2026-08-07) ─────────────────────────────────────
     Les fonctions purement présentationnelles vivent désormais dans
     fh-utils.js et fh-dice-visual.js, chargés avant ce fichier. Les alias
     ci-dessous laissent les appels de ce fichier STRICTEMENT inchangés :
     cette coupe est un déménagement, pas une réécriture.
     Règle à tenir : ne sort d'ici que ce qui ne touche pas `state`. */
  var FHU = window.FH.utils, FHD = window.FH.diceVisual;
  var esc = FHU.esc, clamp = FHU.clamp, numberOr = FHU.numberOr, mod = FHU.mod,
      signed = FHU.signed, pbFor = FHU.pbFor, uuid = FHU.uuid,
      nowLabel = FHU.nowLabel, rollDie = FHU.rollDie;
  var ROLL_DIE_SIZES = FHD.ROLL_DIE_SIZES,
      LIGHTWEIGHT_DICE_THRESHOLD = FHD.LIGHTWEIGHT_DICE_THRESHOLD,
      ROLL_SOURCES = FHD.ROLL_SOURCES, UNKNOWN_SOURCE = FHD.UNKNOWN_SOURCE,
      SEALABLE_SOURCES = FHD.SEALABLE_SOURCES, rollSource = FHD.rollSource,
      HAND_LABELS = FHD.HAND_LABELS,
      DIE_MATERIAL = FHD.DIE_MATERIAL, DIE_COLOURS = FHD.DIE_COLOURS,
      SOURCE_TINT = FHD.SOURCE_TINT, dieMaterialName = FHD.dieMaterialName,
      dieSvg = FHD.dieSvg, pickerFace = FHD.pickerFace, tokenSvg = FHD.tokenSvg,
      dieSize = FHD.dieSize, shortDieLabel = FHD.shortDieLabel,
      handLabel = FHD.handLabel, visualDie = FHD.visualDie;
