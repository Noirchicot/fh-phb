# ARCHITECTE — l'archive du mandat

> **Ce fichier ne porte que l'HISTOIRE.** Il est sorti de `ARCHITECTE.md` le **2026-08-23**,
> quand le mandat pesait **1286 lignes · 72 sections · 78 Ko** et que sa date la plus récente
> était le **2026-08-14**.
>
> ⛔ **Ce n'est pas une poubelle, et ce n'est pas une source.** Tout ce qui est ici a été vrai
> et daté. Rien ici ne se cite comme un fait d'aujourd'hui : les SHA, les comptes de tests, les
> listes de lots en vol et les chemins de fichiers y sont **périmés par construction**.
>
> **Ce qu'on vient y chercher** : *pourquoi* une règle du mandat existe — l'incident qui l'a
> payée. Le mandat porte la règle ; l'archive porte l'incident.
>
> **Les trois voisins** : les **règles** vivent dans `ARCHITECTE.md` · l'**état** vivant dans
> `CHANTIER-STATUS.json` · l'**histoire** ici. Même partage que le tableau de bord, dégonflé le
> 2026-08-20 de 294 Ko vers `CHANTIER-ARCHIVE.json`.

---

## 🔴 Ce qui a été retiré comme FAUX, et qu'on garde ici pour ne pas le réintroduire

**Mesuré le 2026-08-23.** Ces lignes ont vécu dans le mandat ; elles n'étaient plus vraies.

| Ce que le mandat disait | Ce que la mesure dit |
|---|---|
| `FHV2 - ADDENDUMS (source n°1).md` est la **source n°1** des règles de jeu (cité 4 fois) | **Aucun fichier de ce nom dans le vault** (`find -iname "*ADDENDUM*"` → 0). Et le mot est banni du chantier |
| « lis `PASSATION-2026-08-14-SOIR.md`, c'est la plus récente » | La plus récente est **`FHPC — passation architecte (2026-08-21).md`** au logbook, écrite **7 jours** plus tard |
| Quatre documents dans `vault Chantier FH & FHPC/` | **Ce dossier n'existe pas.** Les quatre vivent sous `FH-WEB/FHPC/FHPCv2 *.md` |
| « les comptes rendus vont dans `7.CLAUDE AND ERIC LOGBOOK/Chantier FH & FHPC/` » | Dossier inexistant. Ils vont dans `D&D — Tech & Outils/` |
| `fhpc` = `8d846f6`, **2 commits non poussés** ; `fh-srd` = `20c6598` ✅ à jour | `fhpc` = `5fa853e`, **poussé et à jour** ; `fh-srd` = `1580a2a`, **11 commits NON poussés** |
| « `origin` ne porte que `main`, aucune branche distante à nettoyer » | Vrai pour `fhpc` (1). **`fh-phb` en porte 15**, `fh-srd` **5** |
| « `ui/` n'a AUCUN test — 538 lignes sans filet » | **38 fichiers de tests** citent `ui/builder` ou `shell.css` |
| « `tokens.css` n'a **aucun** `[data-theme]` » (dit 3 fois) | Il en a **1** |
| « 765 · 882 · 906 tests verts » | **1345 tests** aujourd'hui — 1342 verts, **3 rouges** |
| « **Quatre** conseillers » suivi d'un tableau de **six**, puis « un **cinquième** siège : GHOSTWRITER » | Le compte se contredit dans sa propre section |
| `deploy_pages.sh` cité comme le garde du site **PHB** | Le fichier n'existe que dans **`~/tools/fh-srd/`**. `fh-phb` n'a ni script ni workflow : il publie par `mkdocs` + `sync_from_vault.py` |
| « **LE BUILDER EST COMPLET / FINI**, plus aucun placeholder » | L'écran Équipement est en `probleme` au tableau de bord ; l'écran R1 a été fusionné le **23/08** et son cahier des charges garde un §9 « pas tranché » |

📌 **La forme commune des douze** : aucune n'était fausse le jour où elle a été écrite. Elles
sont devenues fausses **sans que rien ne le signale** — c'est la consigne 3 bis du majordome,
*une raison mesurée périme*, vérifiée sur le mandat lui-même.

---

## A. Les pointeurs de reprise du mandat, tels qu'ils étaient au 2026-08-14

> ⛔ **Périmés.** Conservés parce qu'ils disent quelles passations ont été consommées, et
> ce que chacune portait de faux — c'est encore utile pour lire une vieille passation.
...
## B. La hiérarchie des sources ratifiée le 2026-08-10 — RETIRÉE

> ⛔ **Sa source n°1 n'existe pas**, et le mot est banni. Elle a été remplacée le
> **2026-08-18** par la source unique : le chapitre du vault qui alimente le site (voir
> `ARCHITECTE.md` §3). Conservée ici pour son MOTIF, qui reste juste : les décisions d'Eric
...
## C. L'état du chantier au 2026-08-13/14, et les défauts payés qui l'accompagnent

> ⛔ **Aucun SHA, aucun compte de tests, aucune liste de lots en vol de cette section n'est
...
## D. La section « conseillers » telle qu'elle était au 2026-08-14
